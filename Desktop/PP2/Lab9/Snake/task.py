...
class FRUIT:
    ...
    def randomize(self):
        """Randomly places food on the grid with a new timer and weight."""
        self.x = random.randint(0, cell_number - 1)
        self.y = random.randint(0, cell_number - 1)
        self.pos = Vector2(self.x, self.y)
        self.weight = random.choice([1, 2, 3])  # Different food values
        self.timer = pygame.time.get_ticks()  # Reset timer
        self.lifetime = random.randint(3000, 6000)  # Reset lifetime
    def expire(self):
        """Removes food when expired."""
        self.pos = None
...
class MAIN:
    ...
    def check_collision(self):
        if self.fruit.pos == self.snake.body[0]:
            ...
            self.snake.add_block(self.fruit.weight)
    ...
    def check_food_timer(self):
        """Removes food if it exists past its lifetime."""
        if self.fruit.pos and pygame.time.get_ticks() - self.fruit.timer > self.fruit.lifetime:
            self.fruit.expire()
    def game_over(self):
        self.snake.reset()
...
apple = pygame.image.load('images/a.png').convert_alpha()
