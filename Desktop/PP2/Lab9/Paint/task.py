    ...
    for event in pygame.event.get():
        ...
        if line:
            if event.type == pygame.MOUSEBUTTONDOWN:
                draw = True
                last_pos = pos
                pygame.draw.circle(screen, color_mode, pos, 5)
            ...
            if event.type == pygame.MOUSEMOTION:
                if draw:
                    drawLine(screen, last_pos, pos, 5, color_mode)
                last_pos = pos
        ...
        if rectangle:
            if event.type == pygame.MOUSEBUTTONDOWN:
                last_pos = pos
            if event.type == pygame.MOUSEBUTTONUP:
                drawRect(screen, last_pos, pos, 5, color_mode)

        if circle:
            if event.type == pygame.MOUSEBUTTONDOWN:
                last_pos = pos
            if event.type == pygame.MOUSEBUTTONUP:
                drawCircle(screen, last_pos, pos, 5, color_mode)
        ...
        if square:
            if event.type == pygame.MOUSEBUTTONDOWN:
                last_pos = pos
            if event.type == pygame.MOUSEBUTTONUP:
                drawSquare(screen, last_pos, pos, 5, color_mode)

        if rtriagnle:
            if event.type == pygame.MOUSEBUTTONDOWN:
                last_pos = pos
            if event.type == pygame.MOUSEBUTTONUP:
                drawrtriangle(screen, last_pos, pos, 5, color_mode)
